...
#' @param type select type of estimation."fixt" indicates fix time t estimation. "fixx" represents fix variate estimation. The default is "not" which indicates no fix value
...
#'        
#' 
#' @return If option equals to "each". It returns A list containing dataframes with three columns each. The first column represents input values. The second column contains values of the estimated function, 
...
#'         in the second column. If option equals to "all". It returns a list containing 2 lists. The first one contains SCR dataframe and the second one is estimated function by using separating function 
#'         of t and function of x.  
#' @export

sep.test <- function(ts, c, d, m = "MV", b_time, b_timese, mp_type, type = "not", fix_num = 0, option = "each", r = 1, s = 1, n_point = 2000, upper = 10){
  if(option == "each"){
    ...
    res = sepera_3d(ts, c, d, m, b_time, b_timese, mp_type, r = r, s = s, n_point = n_point,  upper = upper)
    return(res)
  } else{
    return(stop("Invalid option!"))
  }
